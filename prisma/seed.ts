...
async function main() {
  ...
  //혼자/함께/반려동물/포토존/체험/굿즈/재미있는/귀여운/힙한/세련된/미니멀/편안한/힐링
  const styleList = [
    {
      idx: 1,
      name: '혼자',
  ...
  for (const style of styleList) {
    await prismaClient.style.upsert({
      where: {
        idx: style.idx,
      },
      update: {
  ...
  for (const inquiryType of inquiryTypeList) {
    await prismaClient.inquiryType.upsert({
      where: {
        idx: inquiryType.idx,
      },
      update: {
