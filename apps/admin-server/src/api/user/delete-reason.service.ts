...
export class DeleteReasonService {
  ...
  getDeleteUserTypeAll: () => Promise<DeleteUserTypeEntity[]> = async () => {
    ...
    return deleteUserTypeList.map((type) =>
      DeleteUserTypeEntity.createEntity(type),
    );
  };
  ...
  getDeleteReasonAll: (pagerble: GetDeleteReasonPagerbleDto) => Promise<{
    ...
    return {
      reasonList: reasonList.map((reason) =>
        DeleteReasonEntity.createEntity(reason),
      ),
      count,
    };
  };

  getDeleteReasonByIdx: (idx: number) => Promise<DeleteReasonEntity> = async (
    idx,
  ) => {
    ...
    if (!deleteUserReason) {
      throw new DeleteUserReasonNotFoundException(
        'Cannot find delete user reason',
      );
    }
