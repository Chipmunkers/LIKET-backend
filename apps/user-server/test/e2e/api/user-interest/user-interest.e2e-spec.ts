...
describe('User Interest (e2e)', () => {
  ...
  describe('POST /user-interest', () => {
    ...
    it('Update test', async () => {
      ...
      expect(
        locationList.map((location) => location.bCode).sort(),
      ).toStrictEqual(dto.locationList.sort());
    });
  ...
  describe('GET /user-interest/all', () => {
    it('Success', async () => {
      const loginUser = test.getLoginUsers().user1;
      ...
      const { body }: { body: InterestTagEntity } = await request(
        test.getServer(),
      )
      ...
      expect(body.ageList.map((age) => age.idx).sort()).toStrictEqual(
        dto.ageList.sort(),
      );
      expect(body.styleList.map((style) => style.idx).sort()).toStrictEqual(
        dto.styleList.sort(),
      );
      expect(body.genreList.map((genre) => genre.idx).sort()).toStrictEqual(
        dto.genreList.sort(),
      );
      expect(
        body.locationList.map((location) => location.bCode).sort(),
      ).toStrictEqual(dto.locationList.sort());
    });

    it('No token', async () => {
      await request(test.getServer()).get('/user-interest/all').expect(401);
    });
  });
});
